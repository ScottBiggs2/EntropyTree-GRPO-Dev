"""MCTS tree node and transition dataclasses for Dream stack."""
...
class MCTSNode:
    ...
    prompt_len: int
    # Cumulative number of denoising steps taken to reach this node.
    step_index: int
    ...
    # Number of denoising steps along the incoming edge (parent -> this).
    # Used for interval-aware time weighting when edges have variable length.
    steps_in_edge: Optional[int] = None
    ...
    def num_masked_tokens(self) -> int:
        """Count mask tokens in the response region only."""
        ...
        return int((response_slice == self.mask_id).sum().item())
...
class TreeTransition:
    ...
    # Start and end step indices for this edge.
    step_index: int
    child_step_index: int
    advantage: float
    entropy: float
    time_weight: float
